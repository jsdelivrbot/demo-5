$(function() {
	...
	var couseIntroduction = {
		init: function() {
			var _this = this;
			_this.courseList();
		},
		courseList: function(curr) {
			var _this = this;
			var curr = curr == undefined ? "1" : curr;
			$.ajax({
				url: "/star/course/list",
				type: "post",
				dataType: "json",
				data: {
					id: institutionId,
					...
					if(data.result == "1") {
						...
						if(data.data.list.length != 0) {
								...
								$.each(data.data.list, function(index, element) {
									courseListStr += '<li class="gotoDetail" data-id="' + element.id + '"><span><img src="' + element.logo + '"/></span><p>' + element.name + '</p></li>'
								});
								$('#btn-more').before(courseListStr);
								laypage({
								...
								$('.gotoDetail').click(function() {
									window.location.href = '../html/courseDetail.html?id=' + $(this).data('id');
								});
						} else {
							seize();
						}
