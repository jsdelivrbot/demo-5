$(function() {	
	...
	$(".btn-code").click(function() {
		if($(".bind-phone").val() == "") {
			layer.open({content: '手机号不得为空',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});
		} else {
			clearInterval(clear);
			if(checkPhone()) {
				timer(intDiff);
		...
		if(!(/^1[3|4|5|7|8]\d{9}$/.test(phone))) {
			...
			$.ajax({
				...
				success: function(data) {
					switch(data.result){
						case "1": alert(data.data.code);timer(intDiff); break;
						case "200": layer.open({content: '用户不存在',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
						case "205": layer.open({content: '手机号已注册',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
						case "206": layer.open({content: '手机验证码操作类型错误',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
	...
	$(".btn-confirm").click(function() {
		...
		if(!(/^1[3|4|5|7|8]\d{9}$/.test(phone))) {
			...
			$.ajax({
				...
				dataType: "json",
				data: {'userId':userId,'token':token,'phone': phone,'passwd': passwd,'code': code},
				success: function(data) {
					switch(data.result){
        				case "1": layer.open({content: '成功绑定手机号',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});
							window.location.reload(); break;
						case "200": layer.open({content: '用户不存在',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
    					case "202": layer.open({content: '手机验证码错误',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
    					case "203": layer.open({content: '用户密码强度不够',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});
    						clearInterval(clear);
    						$('.btn-code').html('发送验证码').removeAttr('disabled');
							$('.code').val("");
							$('.btn-confirm').css("background-color","#605f6a"); break;
    					case "204": layer.open({content: '手机验证码过期',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
    					case "209": layer.open({content: '手机号码已绑定',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
        				default: layer.open({content: '网络出错，请重试',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',time: 2});break;
					}
				},
				error: function() {
