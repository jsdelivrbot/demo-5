			 		...
			 		switch(data.result){
			 			case "1":
				 			...
				 			if(user.isOpenedHomepage == false){
									html += '<div id="filePicker" class="messUser"><div class="messLeft"><span>用户头像</span><p class="uploader-list"><img src="' + user.avatar + '"/></p><i class="iconfont select">&#xe600;</i></div></div>\
			...
			$('.modifyinfo').on('click',function(){				
				...
				$.ajax({
					url:"/star/user/modifyHomepageAccess",
				 	...
				 	success:function(data){
				 		switch(data.result){
				 			case "1": layer.open({content: '修改成功',style: 'background:rgba(0,0,0,.8); color:#fff; border:none;',btn: ['确定'],yes: function(index){window.history.go(-1);}}); break;
