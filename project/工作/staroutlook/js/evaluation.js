$(document).ready(function() {
	...
	var testQuestions, answerTime, totalCount, groupId;
	...
	var evaluation = {
		...
		continueanswer: function() {//继续答题
			var currentPage = localStorage.getItem("currentPage"),
				answeredCount = localStorage.getItem("answeredCount"),
		...
		countDown: function() {//倒计时
			...
			var timer = setInterval(function() {
				...
				surplusTime = $('#timeCounter').text();
				localStorage.setItem('surplusTime', surplusTime);
			}, 1000);
		},
		listening: function() {
			//音频
		...
		reading: function() {
			if($('.answer-active').find('.mui-scroll-wrapper').size() != 0) {
				//阅读类型-图表/文字
				var totalHeight = localStorage.getItem("totalHeight")
		...
		nextPage: function(e) {//下一页
			...
			//如果已做完所有题,就直接提交
			var answeredCount = parseInt(localStorage.getItem("answeredCount"));
			if(!answeredCount) {
				answeredCount = 0;
			}
			...
			answeredCount = parseInt(localStorage.getItem("answeredCount"));
			...
			localStorage.setItem("currentPage",$(".container").index($(".answer-active"))); // 记录当前页
		...
		submitAnswer: function() {//提交答案
			...
			if(answers == null) {
				...
				answerTime = localStorage.getItem('answerTime') + ":00";
	...
	if(!cookie) {
		$.ajax({
			...
			success: function(data) {
				...
				localStorage.setItem("testQuestions", JSON.stringify(data.data.partsQuestions));

				localStorage.setItem("cookie", 1);
			}
		});
	} else {
		answerTime = localStorage.getItem('surplusTime');
		totalCount = localStorage.getItem('totalCount');
