				...
				success: function(data){
					...
					if(data.result == 1) {
						if(data.data.list.length == 0){
							var html = '<div class="backImg"><div class="backImg-star"><img src="../images/sweet.jpg"/></div><span>没有粉丝呢(ㄒoㄒ)~~</span></div>';
							...
							$.each(data.data.list, function(obj, news) {
								html += '<li class="gohome" data-id="'+ news.id +'"><img class="userAvatar" src="' + news.avatar + '">';
								if(news.category == 2||news.category == 3||news.category == 4){
									html += '<i class="iconfont icon-v"></i>';
								}
								html += '<span class="reName">' + news.name +'</span></li>';
							});
							$('#fansTab').append(html);
